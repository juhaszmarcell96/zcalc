...
private:
    ...
    std::size_t count_nonzero_rows (std::size_t start_row, std::size_t end_row, std::size_t start_col, std::size_t end_col, const T& zero_value) {
        std::size_t count = 0;
    ...
    bool get_leftmost_nonzero_indexes(std::size_t start_row, std::size_t* ret_row_index, std::size_t* ret_col_index, const T& zero_value) {
    ...
    bool move_nonzero_row (std::size_t starting_row_index, std::size_t column_index, const T& zero_value) {
    ...
    bool is_row_zero (std::size_t row_index, const T& zero_value) {
        for (std::size_t col_index = 0; col_index < get_num_cols(); ++col_index) {
            if (m_matrix[row_index][col_index] != zero_value) {
                return false;
            }
    ...
    bool are_values_zero (std::size_t row_index, std::size_t col_start, std::size_t col_end, const T& zero_value) {
...
public:
    Matrix (std::size_t num_rows, std::size_t num_cols, const T& fill_value) {
        ...
        m_matrix = std::vector<std::vector<T>>(num_rows, std::vector<T>(num_cols, fill_value));
    }
    Matrix (std::size_t num_rows, std::size_t num_cols) {
        if (num_rows == 0) throw std::invalid_argument("number of rows cannot be 0");
        if (num_cols == 0) throw std::invalid_argument("number of columns cannot be 0");
        m_matrix = std::vector<std::vector<T>>(num_rows, std::vector<T>(num_cols));
    }
    Matrix () = delete;
    ~Matrix () = default;
    ...
    bool solve_system_of_linear_equations(std::vector<T>& solution, const T& zero_value, const T& unit_value) {
        ...
        for (std::size_t row_index = 0; row_index < get_num_rows(); ++row_index) {
            ...
            if (!get_leftmost_nonzero_indexes(row_index, &leftmost_nonzero_row_index, &leftmost_nonzero_col_index, zero_value)) {
            ...
            m_matrix[row_index][leftmost_nonzero_col_index] = unit_value;
            ...
            for (std::size_t i = row_index + 1; i < get_num_rows(); ++i) {
                T multiplier = m_matrix[i][leftmost_nonzero_col_index] / m_matrix[row_index][leftmost_nonzero_col_index];
                for (std::size_t j = 0; j < get_num_cols(); ++j) {
        ...
        for (row_index; row_index >= 0; --row_index) {
            if (are_values_zero(row_index, 0, get_num_cols() - 1, zero_value)) continue;
            ...
            for (int backsub_row_index = row_index - 1; backsub_row_index >= 0; --backsub_row_index) {
                T multiplier = m_matrix[backsub_row_index][leading_one_index] / m_matrix[row_index][leading_one_index];
        ...
        std::size_t rank_A  = count_nonzero_rows(0, get_num_rows() - 1, 0, get_num_cols() - 2, zero_value);
        std::size_t rank_Ab = count_nonzero_rows(0, get_num_rows() - 1, 0, get_num_cols() - 1, zero_value);
