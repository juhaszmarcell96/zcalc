...
LinearEquationSystem::LinearEquationSystem (std::size_t num_variables) : m_num_variables (num_variables) {}

bool LinearEquationSystem::append_equation (const LinearEquation<Complex> & equation) {
    if (equation.get_num_variables() != m_num_variables) return false;
    m_linear_equation_system.push_back(equation);
    return true;
}

std::size_t LinearEquationSystem::get_num_variables () const { return m_num_variables; }

std::size_t LinearEquationSystem::get_num_equations () const { return m_linear_equation_system.size(); }

void LinearEquationSystem::exchange_equations (const std::size_t i, const std::size_t j) {
    if (i == j) return;
...
bool LinearEquationSystem::get_leftmost_nonzero_indexes(std::size_t start_equ, std::size_t* ret_equ_index, std::size_t* ret_var_index) {
...
bool LinearEquationSystem::is_solvable () {
...
bool LinearEquationSystem::forward_elimination () {
...
bool LinearEquationSystem::back_substitution () {
...
bool LinearEquationSystem::solve(std::vector<Complex>& solution) const {
...
std::ostream& operator<<(std::ostream& os, const LinearEquationSystem& system) {
